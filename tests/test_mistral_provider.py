"""
Ethicore Engine™ - Guardian SDK — Mistral AI Provider Tests
Version: 1.0.0
...
Covers:
  ...
  - Exception hierarchy
  - Constants (base URL, model set including versioned and alias models)
...
from ethicore_guardian.providers.mistral_provider import (
    MISTRAL_BASE_URL,
    MISTRAL_MODELS,
...
def _make_mistral_client(base_url: str = MISTRAL_BASE_URL) -> MagicMock:
    """Build a mock OpenAI client configured for Mistral AI."""
    client = MagicMock()
...
# ===========================================================================
# 1. MistralProvider — client detection
# ===========================================================================

class TestMistralClientDetection:
    def setup_method(self):
        self.provider = MistralProvider(_make_guardian())

    def test_detects_mistral_base_url(self):
        client = _make_mistral_client("https://api.mistral.ai/v1")
        assert self.provider._is_mistral_client(client)

    def test_detects_mistral_in_url_string(self):
        client = _make_mistral_client("https://mistral-proxy.internal/v1")
        assert self.provider._is_mistral_client(client)

    def test_detects_via_alt_base_url_attribute(self):
        client = MagicMock()
        client.base_url = ""
        client._base_url = "https://api.mistral.ai/v1"
        assert self.provider._is_mistral_client(client)

    def test_rejects_openai_base_url(self):
        client = _make_mistral_client("https://api.openai.com/v1")
        assert not self.provider._is_mistral_client(client)
...
# ===========================================================================
# 2. MistralProvider — prompt extraction
# ===========================================================================
...
class TestPromptExtraction:
    ...
    def test_extracts_multimodal_content_blocks(self):
        ...
        assert "Summarise this document" in text
...
# ===========================================================================
# 3. MistralProvider — tool result extraction
# ===========================================================================
...
class TestPromptScanPolicy:
    def _make_completions(self, verdict="ALLOW", strict_mode=False) -> ProtectedCompletions:
        ...
        provider = MistralProvider(guardian)
        original = MagicMock()
...
# ===========================================================================
# 9. ProtectedMistralClient — transparent delegation
# ===========================================================================

class TestProtectedMistralClientDelegation:
    def test_wraps_chat_completions(self):
        guardian = _make_guardian()
        client = _make_mistral_client()
        protected = ProtectedMistralClient(client, guardian)
    ...
    def test_delegates_unknown_attrs_to_original(self):
        guardian = _make_guardian()
        client = _make_mistral_client()
        client.some_attr = "mistral_value"
        protected = ProtectedMistralClient(client, guardian)
        assert protected.some_attr == "mistral_value"

    def test_repr_identifies_wrapper(self):
        guardian = _make_guardian()
        client = _make_mistral_client()
        protected = ProtectedMistralClient(client, guardian)
        assert "ProtectedMistralClient" in repr(protected)


# ===========================================================================
# 10. Mistral constants and model set
# ===========================================================================

class TestConstants:
    def test_base_url_points_to_mistral(self):
        assert "mistral.ai" in MISTRAL_BASE_URL
        assert MISTRAL_BASE_URL.startswith("https://")

    def test_base_url_includes_version_path(self):
        # Mistral uses /v1 path unlike bare domain providers
        assert "/v1" in MISTRAL_BASE_URL

    def test_model_set_contains_flagship(self):
        assert "mistral-large-3-25-12" in MISTRAL_MODELS

    def test_model_set_contains_code_specialist(self):
        assert "codestral-25-08" in MISTRAL_MODELS

    def test_model_set_contains_agentic_model(self):
        assert "devstral-2-25-12" in MISTRAL_MODELS

    def test_model_set_contains_short_aliases(self):
        assert "mistral-large-latest" in MISTRAL_MODELS
        assert "codestral-latest" in MISTRAL_MODELS

    def test_model_set_is_comprehensive(self):
        # Should include frontier, reasoning, efficient, code, and moderation tiers
        assert len(MISTRAL_MODELS) >= 10
