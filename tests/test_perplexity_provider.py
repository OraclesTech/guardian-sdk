"""
Ethicore Engine™ - Guardian SDK — Perplexity Provider Tests
Version: 1.0.0
...
Covers:
  ...
  - Exception hierarchy
  - Constants (base URL, Sonar model set)

Note on web grounding: All Sonar models retrieve live web content as part of
inference. Guardian scans the user-supplied prompt before it reaches Perplexity
(Layer 1), but does NOT scan Perplexity's internal web fetch results — that
surface is outside the API call boundary. These tests cover the surfaces
Guardian *does* protect.
...
from ethicore_guardian.providers.perplexity_provider import (
    PERPLEXITY_BASE_URL,
    PERPLEXITY_MODELS,
...
def _make_perplexity_client(base_url: str = PERPLEXITY_BASE_URL) -> MagicMock:
    """Build a mock OpenAI client configured for Perplexity."""
    client = MagicMock()
...
# ===========================================================================
# 1. PerplexityProvider — client detection
# ===========================================================================

class TestPerplexityClientDetection:
    def setup_method(self):
        self.provider = PerplexityProvider(_make_guardian())

    def test_detects_perplexity_base_url(self):
        client = _make_perplexity_client("https://api.perplexity.ai")
        assert self.provider._is_perplexity_client(client)

    def test_detects_perplexity_in_url_string(self):
        client = _make_perplexity_client("https://perplexity-proxy.internal/v1")
        assert self.provider._is_perplexity_client(client)

    def test_detects_via_alt_base_url_attribute(self):
        client = MagicMock()
        client.base_url = ""
        client._base_url = "https://api.perplexity.ai"
        assert self.provider._is_perplexity_client(client)

    def test_rejects_openai_base_url(self):
        client = _make_perplexity_client("https://api.openai.com/v1")
        assert not self.provider._is_perplexity_client(client)
...
# ===========================================================================
# 2. PerplexityProvider — prompt extraction
# ===========================================================================
...
class TestPromptExtraction:
    ...
    def test_extracts_user_message(self):
        ...
        assert "What happened in AI this week?" in text

    def test_extracts_system_message(self):
        text = self.provider.extract_prompt(
            messages=[
                {"role": "system", "content": "You are a research assistant."},
                {"role": "user", "content": "Summarise recent LLM papers"},
            ]
        )
        assert "You are a research assistant." in text
        assert "Summarise recent LLM papers" in text
    ...
    def test_extracts_multimodal_content_blocks(self):
        text = self.provider.extract_prompt(
            messages=[{
                "role": "user",
                "content": [
                    {"type": "text", "text": "Analyse this source"},
                    {"type": "image_url", "image_url": {"url": "data:..."}},
                ],
            }]
        )
        assert "Analyse this source" in text
...
# ===========================================================================
# 3. PerplexityProvider — tool result extraction
# ===========================================================================
...
class TestPromptScanPolicy:
    def _make_completions(self, verdict="ALLOW", strict_mode=False) -> ProtectedCompletions:
        ...
        provider = PerplexityProvider(guardian)
        original = MagicMock()
...
# ===========================================================================
# 9. ProtectedPerplexityClient — transparent delegation
# ===========================================================================

class TestProtectedPerplexityClientDelegation:
    def test_wraps_chat_completions(self):
        guardian = _make_guardian()
        client = _make_perplexity_client()
        protected = ProtectedPerplexityClient(client, guardian)
    ...
    def test_delegates_unknown_attrs_to_original(self):
        guardian = _make_guardian()
        client = _make_perplexity_client()
        client.some_attr = "perplexity_value"
        protected = ProtectedPerplexityClient(client, guardian)
        assert protected.some_attr == "perplexity_value"

    def test_repr_identifies_wrapper(self):
        guardian = _make_guardian()
        client = _make_perplexity_client()
        protected = ProtectedPerplexityClient(client, guardian)
        assert "ProtectedPerplexityClient" in repr(protected)


# ===========================================================================
# 10. Perplexity constants and Sonar model set
# ===========================================================================

class TestConstants:
    def test_base_url_points_to_perplexity(self):
        assert "perplexity.ai" in PERPLEXITY_BASE_URL
        assert PERPLEXITY_BASE_URL.startswith("https://")

    def test_model_set_contains_sonar(self):
        assert "sonar" in PERPLEXITY_MODELS

    def test_model_set_contains_sonar_pro(self):
        assert "sonar-pro" in PERPLEXITY_MODELS

    def test_model_set_contains_reasoning_model(self):
        assert "sonar-reasoning-pro" in PERPLEXITY_MODELS

    def test_model_set_contains_deep_research(self):
        assert "sonar-deep-research" in PERPLEXITY_MODELS

    def test_model_set_covers_all_sonar_variants(self):
        # All four Sonar variants should be present (May 2026 catalog)
        assert len(PERPLEXITY_MODELS) >= 4
